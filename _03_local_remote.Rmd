...
```sh
$ git@github.com:inSilecoInc/demogit.git
```

- `git clone` get a copy of the remote repo (`git`);
- set up a local branch to track the the default remote branch. 

--

### 2. Fork 

- Copy a `r gh()` repository on one of you ` rgh()` account.


???
delete add re-initiate proj1 with a readme and a .gitignore for clone
then do a fork and then a cloe
...
### `nf()` Having a local repo and a remote repo is simply about tracking more branches.
